...
- 🟢 **Rate Limit & Caching:** `rate_limit_service.py` ve `cache_store.py` modülleri in-memory yapıda stabil hale getirildi.
...
- 🟢 **Repository Pattern:** `backend/core/db.py` içerisinde ham SQLite sorguları güvenli WAL modunda çalışacak şekilde optimize edildi.
...
1. **Veritabanı İyileştirmesi:** SQLite `WAL` modunda timeout ayarlarıyla optimize edilerek concurrent okuma/yazma performansı artırıldı.
...
3. **Session Cache & Rate Limiting:** `TTLLRUCache` tabanlı in-memory cache mekanizmaları `cache_store.py` ve `rate_limit_service.py` içerisinde aktif edildi.
...
- In-memory TTLLRUCache ve Rate Limit (Aktif)
...
Son yapılan SQLite stabilizasyonları ve in-memory mimari iyileştirmeleriyle birlikte Lingofy stabil bir MVP durumuna geldi.
...
Fikir ve pazardaki boşluk mükemmel. Son stabilizasyonlar (Rate limit, Auth güvenliği) sayesinde Lingofy lansmana hazır hale geldi.
