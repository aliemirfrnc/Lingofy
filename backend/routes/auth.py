...
def _set_auth_cookies(response: Response, access_token: str, refresh_token: str):
    ...
    response.set_cookie(key="access_token", value=access_token, httponly=True, secure=IS_PRODUCTION, samesite="lax", max_age=900)
    response.set_cookie(key="refresh_token", value=refresh_token, httponly=True, secure=IS_PRODUCTION, samesite="lax", max_age=2592000)
...
def logout_all(request: Request, response: Response, authorization: str | None = Header(default=None)):
    ...
    try:
        ...
        # Fallback to refresh token to identify user
        refresh_token = request.cookies.get("refresh_token")
        if refresh_token:
