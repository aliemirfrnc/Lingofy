...
_ai_cache = get_cache("pronunciation_ai", ttl_seconds=600)
...
async def generate_ai_feedback(tech_data: Dict[str, Any], context: Dict[str, Any], language: str = "tr") -> Dict[str, Any]:
    ...
    cached_data = _ai_cache.get(cache_key)
    if cached_data:
        return cached_data
...
async def analyze_pronunciation(
    ...
    if audio.size is not None and audio.size == 0:
        raise HTTPException(status_code=400, detail="Ses dosyası boş olamaz.")
        
    if audio.size is not None and audio.size > 2 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Ses dosyası boyutu 2MB'ı geçemez.")
        
    allowed_mimes = ["audio/webm", "audio/mp3", "audio/mpeg", "audio/wav", "audio/ogg", "audio/x-wav", "video/webm"]
    if audio.content_type not in allowed_mimes:
        raise HTTPException(status_code=400, detail=f"Geçersiz dosya formatı: {audio.content_type}")
    ...
    return AIResponse(
        ...
        summary=ai_data.get("summary") or "İyi iş çıkardın!",
        strengths=ai_data.get("strengths") or ["Teknik okuma başarılı"],
        weaknesses=ai_data.get("weaknesses") or ["Gelişim alanları belirleniyor..."],
        suggestions=ai_data.get("suggestions") or ["Daha fazla pratik yap."],
        next_goal=ai_data.get("next_goal") or "Yola devam!",
        insight=ai_data.get("insight") or "Harikasın!",
        xp_gained=xp_gained,
        level_up=level_up
    )
