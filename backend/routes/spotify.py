...
# Eşzamanlı token yenilemesini önlemek için 256 adet sabit (striped) kilit (Memory Leak'i önler)
_refresh_locks = [threading.Lock() for _ in range(256)]
...
def _get_refresh_lock(user_id: int) -> threading.Lock:
    return _refresh_locks[hash(user_id) % 256]
