...
_rate_limit = get_cache("translate_limits", ttl_seconds=86400)
...
async def _check_rate_limit(user_id: int, cost: int = 1) -> None:
    async with _rate_limit_lock:
        ...
        try:
            last_date, count = _rate_limit.get(str(user_id), (today, 0))
        except KeyError:
        ...
        _rate_limit.set(str(user_id), (today, count + cost))
...
async def _translate_cached(text: str, track_id: str | None = None) -> tuple[str | None, bool]:
    ...
    if len(text_strip) > 500:
        raise HTTPException(status_code=400, detail="Çevrilecek metin çok uzun (Maksimum 500 karakter).")
    ...
    async with _translate_semaphore:
        try:
            ...
            if result and result.startswith('"') and result.endswith('"') and len(result) >= 2:
                result = result[1:-1]
            if result:
                result = result.strip()
