...
async def word_info(
    ...
    if not word or len(word) > 100 or len(payload.context_line) > 500:
        raise HTTPException(status_code=400, detail="Geçersiz kelime veya bağlam.")
    ...
    async def fetch_all():
        d_data, trans, a_data = await asyncio.gather(
            DictionaryService.get_word_definition(word),
            TranslationService.get_turkish_translation(word),
            _get_ai_context()
        )
        return d_data, trans, a_data

    # Cache Stampede Protection (Duplicate Requests Prevention)
    if cache_key in _active_tasks:
        dict_data, translation, ai_data = await _active_tasks[cache_key]
    else:
        task = asyncio.create_task(fetch_all())
        _active_tasks[cache_key] = task
        try:
            dict_data, translation, ai_data = await task
        finally:
            _active_tasks.pop(cache_key, None)
    ...
    # Background save is handled by TTLLRUCache or periodic job
